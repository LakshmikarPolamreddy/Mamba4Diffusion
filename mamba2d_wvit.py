...
class Mamba2DModel(Transformer2DModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Collecting the skip list with all blocks having skips (True for every block)
        collect_skip_list = [True for d in range(num_layers)]  # All blocks will have skip connections

        # Assigning skip tuning parameters
        skip_top_val, skip_bottom_val = skip_tune_param
        skip_bottom_id, skip_top_id = num_layers, -1

        # Calculate skip_bottom_id and skip_top_id based on collect_skip_list
        for d in range(num_layers):
            if collect_skip_list[d]:  # All blocks will have skip
                skip_bottom_id = min(skip_bottom_id, d)
                skip_top_id = max(skip_top_id, d)

        # Assign final values for skip ids and tuning values
        self.skip_bottom_id = skip_bottom_id
        self.skip_top_id = skip_top_id
        self.skip_top_val = skip_top_val
        self.skip_bottom_val = skip_bottom_val
...
# Version-2:
        ...
        self.transformer_blocks.extend(
            nn.ModuleList(
            [
                BasicMambaBlock(
                    is_skip=collect_skip_list[d],
                    stage_index=d,
                    is_adapt_q=( is_adapt_q_list[d]),
                    use_conv1d=( use_conv1d_list[d]),
                    drop_path=inter_dpr[d],
...
# Until here
    ...
    def forward(
        ...
        for blk_id, block in enumerate(self.transformer_blocks):
            # print(f"Length of is_skip_list: {len(self.is_skip_list)}, Number of transformer blocks: {len(self.transformer_blocks)}")
            # print(f"blk_id: {blk_id}, is_skip_list: {self.is_skip_list}")            
            pre_skip = None 
